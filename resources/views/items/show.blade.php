                                ...
                                @foreach($item->images as $image)
                                    <div class="aspect-square bg-gray-100 rounded-lg overflow-hidden">
                                        <img src="{{ Storage::url('items/' . $item->id . '/' . $image->filename) }}" 
                                             ...
                                             onclick="openImageModal('{{ Storage::url('items/' . $item->id . '/' . $image->filename) }}')">
                                    </div>
                                @endforeach
                            </div>
                        </div>
                            ...
                            <div class="bg-gray-50 rounded-lg p-4">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <span class="ml-2 text-sm text-gray-900">{{ $item->created_at->format('M j, Y \a\t g:i A') }}</span>
                                    </div>
                                    
                                    @if($item->verified_at)
                                        <div>
                                            <span class="text-sm font-medium text-gray-500">Verified:</span>
                                            <span class="ml-2 text-sm text-gray-900">{{ $item->verified_at->format('M j, Y \a\t g:i A') }}</span>
                                        </div>
                                    @endif
                                    ...
                                    @if($item->resolved_at)
                                        <div>
                                            ...
                                            <span class="ml-2 text-sm text-gray-900">{{ $item->resolved_at->format('M j, Y \a\t g:i A') }}</span>
                                        </div>
                                    @endif
                                </div>
                            </div>
                                    ...
                                    <div class="space-y-2">
                                        @if(in_array($method, ['email', 'both']) && !empty($contactInfo['email']))
                                            <div class="flex items-center">
                                                ...
                                                <a href="mailto:{{ $contactInfo['email'] }}" class="text-emerald-700 hover:text-emerald-800 text-sm">
                                                    {{ $contactInfo['email'] }}
                                                </a>
                                            </div>
                                        @endif
                                        ...
                                        @if(in_array($method, ['phone', 'both']) && !empty($contactInfo['phone']))
                                            <div class="flex items-center">
                                                ...
                                                <a href="tel:{{ $contactInfo['phone'] }}" class="text-emerald-700 hover:text-emerald-800 text-sm">
                                                    {{ $contactInfo['phone'] }}
                                                </a>
                                            </div>
                                        @endif
                            ...
                            @if($item->user_id === auth()->id())
                                <div class="space-y-3">
                                    @if($item->status === 'pending')
                                        <a href="{{ route('items.edit', $item) }}" class="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm font-medium text-center block transition duration-150 ease-in-out">
                                            Edit Item
                                        </a>
                                        ...
                                        <form action="{{ route('items.destroy', $item) }}" method="POST" 
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md text-sm font-medium transition duration-150 ease-in-out">
                                                Delete Item
                                            </button>
                                        </form>
                                    @endif
                                    
                                    <a href="{{ route('items.my-items') }}" class="w-full bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-md text-sm font-medium text-center block transition duration-150 ease-in-out">
                                        Back to My Items
                                    </a>
                                </div>
                            @endif
